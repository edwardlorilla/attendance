        ...
        <div class="layout">
            <div class="layout-topbar clearfix">
                <div class="layout-name" :class="{'sider-mini': isCollapse}">
                    <router-link class="full" to="/" v-if="!isCollapse">Payroll System</router-link>
                    <span class="mini" v-else>P</span>
                    </div>
                <div class="layout-collapse" @click="toggleSider"><i class="fa fa-bars"></i></div>
                <div class="layout-nav">
                    <div v-loading.fullscreen.lock="$store.state.loading" @click="handleLogout"><i class="fa fa-sign-out" aria-hidden="true"></i></div>
                    </div>
            ...
            <aside class="layout-sider">
                ...
                <el-scrollbar style="height: calc(100% - 116px);">
                    <el-menu :unique-opened="true"
                             ...
                             :router="true">
                        <component v-for="(item, index) in nav" v-if="item.can ? can[item.can] : true" :key="index" :is="item.type" :index="item.data.route"
    ...
    export default {
        ...
        methods: {
            ...
            handleCommand(command, index) {
                var vm = this
                vm.$router.push({name: 'edit-publication', params: {id: command.data.publication_id}},
                    function () {
                    vm.handleCallback(vm, command, index)
                    })
            },
            handleLogout() {
                var vm = this
